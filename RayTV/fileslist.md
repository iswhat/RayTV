...
| ServiceFactory.ets | data/service/ServiceFactory.ets | 服务工厂，创建和管理服务实例 | ✓ | ✓ | |
...
| 文件名 | 路径 | 功能描述 | 存在于规划文档 | 存在于计划文档 | 备注 |
|-------|------|---------|-------------|-------------|------|
| DownloadService.ts | services/DownloadService.ts | 下载服务（重复实现） | ✓ | ✓ | 与data/service/DownloadService.ets功能重复 |
| LiveStreamService.ts | services/LiveStreamService.ts | 直播流服务（重复实现） | ✓ | ✓ | 与data/service/LiveStreamService.ets功能重复 |
| PlaybackService.ts | services/PlaybackService.ts | 播放服务（重复实现） | ✓ | ✓ | 与data/service/PlaybackService.ets功能重复 |
| UserService.ts | services/UserService.ts | 用户服务（重复实现） | ✓ | ✓ | 与data/service/UserService.ets功能重复 |

## 5. 工具类（额外） (utils/)

| 文件名 | 路径 | 功能描述 | 存在于规划文档 | 存在于计划文档 | 备注 |
|-------|------|---------|-------------|-------------|------|
| CacheService.ts | utils/CacheService.ts | 缓存服务（重复实现） | ✓ | ✓ | 与data/service/CacheService.ets功能重复 |
...
- 建议：统一使用 common/util/ 作为工具类的标准目录，移除 utils/ 中的重复实现

### 2. 服务层重复
- data/service/ 下的服务与 services/ 下的服务存在重复
- 建议：统一使用 data/service/ 作为服务层的标准目录，移除 services/ 中的重复实现

### 3. 功能职责重复
- 例如 HistoryService.ets 和 HistoryManager.ets 功能重叠
- 建议：合并功能，保留一个主要实现
