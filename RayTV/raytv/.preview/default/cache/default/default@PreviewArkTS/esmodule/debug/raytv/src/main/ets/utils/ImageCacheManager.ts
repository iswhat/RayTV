import MemoryManagerClass from "@bundle:com.raytv.app/raytv/ets/common/util/MemoryManager";
/**
 * MemoryManager 接口
 */
interface MemoryManager {
    checkMemoryAvailability: (requiredMemoryMB?: number) => boolean;
    recordLargeObject: (key: string, sizeMB: number) => void;
}
/**
 * 图片缓存管理器
 * 提供LRU缓存策略和内存管理
...
export class ImageCacheManager {
    ...
    private constructor(maxSize: number = 50) {
        ...
        this.currentSize = 0;
        // 初始化 MemoryManager | Initialize MemoryManager
        try {
            this.memoryManager = MemoryManagerClass.getInstance() as MemoryManager;
        }
        catch (error) {
            const err: Error = error instanceof Error ? error : new Error(String(error));
            console.warn('Failed to initialize MemoryManager:', err);
        }
    }
     ...
     * 设置缓存项
     */
    set(key: string, data: string, ttl: number = 300000): void {
        try {
            // 检查内存可用性
     ...
     * 设置错误缓存
     */
    setError(key: string, error: string, ttl: number = 60000): void {
        const entry: CacheEntry = {
            data: '',
            timestamp: Date.now(),
            lastAccessed: Date.now(),
    ...
    clearOldCache(): void {
        ...
        for (const entry of entries) {
            ...
            if (cacheEntry.lastAccessed < oneHourAgo) {
