...
- **开发语言**：ArkTS (主要)、TypeScript (只有在ArkTS无法实现的情况才考虑使用)  优先使用ArkTS作为主要开发语言，仅在必要时使用TypeScript作为辅助语言
  强制使用静态类型
ArkTS要求所有类型在编译时已知，这不仅体现在禁止any和unknown上，还包括‌禁止在运行时变更对象布局‌。这意味着你不能向对象动态添加或删除属性，也不能将任意类型的值赋给对象属性。同时，‌运算符的语义也受到限制‌，例如一元运算符+只能作用于数值类型，对字符串使用会导致编译错误。

类型系统与语法调整
ArkTS在类型系统的设计上做出了显著调整。它‌不支持structural typing‌，这意味着类型的兼容性判断标准与TypeScript不同。此外，‌不支持Symbol() API‌，因为其在静态类型语言中的常见使用场景意义不大。在语法层面，‌必须使用let而非var‌来声明变量。

类与接口的规范
在面向对象编程方面，ArkTS要求‌使用class而非具有call signature（调用签名）的类型‌。对于私有字段，‌不支持以#开头的语法‌，应改用private关键字。接口中也‌不支持构造签名‌。

泛型与高级类型
ArkTS对泛型的使用有更严格的要求。在调用泛型函数时，‌需要显式标注泛型类型实参‌。同时，一些TypeScript中的高级类型特性在ArkTS中‌不被支持‌，例如‌条件类型‌、‌this类型‌和‌索引访问类型‌。

字面量与函数
对于对象和数组字面量，ArkTS规定‌对象字面量不能用于类型声明‌，并且‌数组字面量必须仅包含可推断类型的元素‌。在函数方面，推荐‌使用箭头函数而非函数表达式‌。

这些约束共同确保了ArkTS应用在HarmonyOS上的高性能与高可靠性。
      ArkTS的设计理念：在保持TypeScript开发体验的同时，通过静态类型和明确的装饰器来提升运行时性能
    0、在ArkTS中，不能使用unknown类型，也不能使用any类型，ArkTS要求开发者显式指定具体的类型，而不是使用any或unknown这种宽泛的类型。
    1、由于ArkTS基于TypeScript，但为了性能进行了约束：
      ‌使用动态类型特性‌：ArkTS不支持any、union等动态类型，使用这些类型会导致编译错误
      ‌忽略类型初始化‌：@State装饰器标记的变量必须初始化，不能为空值，这与某些语言中变量可缺省初始化的习惯不同
    2、状态管理的
      不要错误使用多个装饰器‌：同一个变量不能被多个状态管理装饰器（如@State、@Link等）同时装饰。
      ‌@Builder方法参数误用‌：在@Builder方法中，如果参数在初始化时就确定了值，且参数不是响应式的，状态变化时不会触发重新计算，导致视图不更新。正确的做法是直接在@Builder中访问响应式状态变量。
      ‌嵌套对象更新问题‌：嵌套类型以及数组中的对象属性变更可能无法触发视图更新，需要特别注意。
    3、异步处理和事件监听
      ‌事件监听器需要正确移除‌，否则这可能导致内存泄漏，需要确保在组件不再需要时移除事件监听器。
      ‌错误处理不完善‌：异步操作中的错误需要被有效捕获和处理，否则会影响应用稳定性。
    4、装饰器使用规范
      ‌@Watch回调函数定义‌：在struct内定义了@Watch装饰器装饰的变量后，必须定义对应名称的监听函数，否则会报错。
      ‌@BuilderParam初始化‌：@BuilderParam装饰的变量只能被@Builder装饰的函数初始化，使用普通函数或其他类型变量初始化会导致错误。
    5、@State/@Prop/@Link/@ObjectLink/@StorageLink/@LocalStorageProp等装饰器变量的类型必须与初始化值的类型一致，否则会导致编译错误。

    6、在ArkTS中，当需要替代any或unknown类型时，你可以根据具体情况选择以下更具体的类型，这些替代方案旨在保持代码的类型安全

      使用具体的基础类型
      如果变量的值是明确的原始类型（如布尔值、数字、字符串），应该直接使用这些基础类型进行声明。

      ‌布尔值‌：使用 boolean 类型。
      ‌数字‌：使用 number 类型。
      ‌字符串‌：使用 string 类型。
      使用 Object 或 object 类型
      当值的类型不确定，但可以确定它是一个对象时，可以使用 Object 或 object 类型。
      ‌大写的 Object‌：这是一个更宽泛的类型，可以表示任何对象。
      ‌小写的 object‌：它代表非原始类型的对象（即不是 number, string, boolean, bigint, symbol, null, 或 undefined）。在不确定具体对象结构时，Object 是一个常见的选择。
      使用自定义类或接口
      对于具有特定结构或行为的复杂对象，最佳实践是定义具体的‌类 (class)‌ 或‌接口 (interface)‌ 来明确其类型。这种方式最能体现ArkTS静态类型检查的优势，有助于提升代码质量和开发效率。

      处理特定场景（如错误捕获）
      在一些特定场景下，例如在 try-catch 语句中捕获错误，ArkTS要求为错误变量指定明确的类型，通常可以使用内置的 Error 类型。

      总而言之，在ArkTS中，通过使用具体的基础类型、Object/object 类型，或者定义自定义的类和接口，你可以有效地替代 any 和 unknown，从而编写出更安全、更易维护的代码。



    TypeScript开发注意事项：
    1、变量必须显式声明静态类型，禁止any隐式推导。不支持‌Structural typing‌（结构类型系统），需严格依赖接口或类定义对象结构‌
    2、一元运算符（如+）仅允许作用于数字类型
    3、private修饰符的严格约束‌ @State/@Prop等装饰变量用private修饰时，‌禁止在构造函数外赋值‌
    4、禁止public修饰特定装饰器变量‌
    @StorageLink/@LocalStorageProp等变量使用public会触发编译警告‌。
    @Link/@ObjectLink‌不可‌与private共用‌
    5、protected全场景禁用‌，因ArkTS组件‌无继承机制‌，所有变量用protected修饰均会告警‌
    6、交叉类型（&）不支持‌，需改用extends实现类型扩展
    7、不支持可选链操作符（?.），需使用条件判断替代
    8、不支持空值合并操作符（??），需使用条件判断替代
    9、枚举（Enum）被视为不良实践‌，官方建议用‌联合类型‌替代，避免枚举值变更引发的全局修改成本。
    10、不支持类的实例化，所有类均需定义为静态类（static class），禁止使用new关键字创建实例。
    11、‌禁止运行时修改对象布局‌，对象属性不可动态增删，确保内存稳定‌
    12、不支持动态加载类（如Class.forName），需在编译时确定所有依赖类
    13、null/undefined需显式处理‌，变量未赋值时默认为undefined，赋值null需声明联合类型：
    14、先使用联合类型替代枚举，严格限定装饰器变量的访问权限，并通过接口/类明确定义对象结构以规避结构类型缺失问题‌
    15、动态属性扩展禁用‌，对象创建后禁止动态添加/删除属性：
    16、类型断言强制 as 语法‌，禁止使用尖括号类型断言，必须改用 as
    17、函数参数重赋值限制‌、禁止在函数内修改参数值
    18、禁用隐式 any‌，变量必须显式声明类型，禁止隐式 any 推导
    19、禁用 var 关键字‌，强制使用 let/const，var 触发编译错误
    20、对象字面量简写强制启用‌、禁止冗余键值对：
    - 不允许在对象字面量中使用重复的键名
    - 不允许在对象字面量中使用计算属性名
